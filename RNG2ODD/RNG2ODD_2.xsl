...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:loc="http:/local-namespace/"
    ...
    <xsl:template match="element">
        ...
        <elementSpec ident="{$element-name}" module="EAC" xsl:exclude-result-prefixes="loc a xhtml xd">
            ...
            <!-- Attributes -->
            <xsl:if
                test="ancestor::grammar//element[@name = $element-name]/descendant::attribute">
                <attList>
                    <!-- Pulled from attlist.{@name} -->
                        
                    <xsl:for-each select="descendant::attribute[not(some $x in (current()/descendant::* intersect ./ancestor::*) satisfies name($x)='element')]">
                    ...
                    <xsl:when test="$handle-models">
                        <xsl:choose>
                            <xsl:when test="self::ref and starts-with(@name, 'model.')">
                                ...
                                <xsl:attribute name="name">
                                    ...
                                    <xsl:for-each
                                        select="ancestor::define[@name = $this-mdl][not(preceding::define[@name = $this-mdl])]">
                                        <!-- Determine suffix -->
                        ...
                        <xsl:text>_alternation</xsl:text>
                                            ...
                                            <xsl:otherwise>
                                                ...
                                                <xsl:value-of
                                                  select="substring-after(@name, 'model.')"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:for-each>
                                </xsl:attribute>
      ...
      <xsl:text>RNGSchema="http://www.tei-c.org/release/xml/tei/custom/schema/relaxng/tei_odds.rng" type="xml"</xsl:text>
        ...
        <TEI xsl:exclude-result-prefixes="loc a xhtml xd">
            ...
            <teiHeader>
                ...
                <revisionDesc>
                    <change who="#RV" when="{current-date()}"> Converted RNG Datatypes Declarations
                        into ODD Datatype Macros according to TEI guidelines. <ref
                            target="http://www.tei-c.org/release/doc/tei-p5-doc/html/ST.html#DTYPES"
                            >Chapter 1, §1.4.2</ref>. </change>
                    <change who="#RV" when="{current-date()}"> Converted RNG Attribute Classes
                        Declarations into ODD Attribute Classes according to TEI guidelines. <ref
                    ...
                    <change who="#RV" when="{current-date()}"> Converted RNG Element Declarations
                        into ODD Element Declarations according to TEI guidelines. <ref
                    ...
                    <change who="#RV" when="{current-date()}">Integrated Schematron rules within
                        element declarations.</change>
                    <change who="#RV" when="{current-date()}">Integrated modules.</change>
                    <change who="#CR" when="{current-date()}">Adaptation for the EAC-CPF relaxNG
                        schema</change>
                </revisionDesc>
            </teiHeader>
            <text>
...
<!--                    <xsl:apply-templates select="//element[not(parent::define)]"/>-->
        ...
        <xsl:text>RNGSchema="http://www.tei-c.org/release/xml/tei/custom/schema/relaxng/tei_all.rng" type="xml"</xsl:text>
            ...
            <TEI xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns="http://www.tei-c.org/ns/1.0"
                xsl:exclude-result-prefixes="loc a xhtml xd">
                <teiHeader>
                    <fileDesc>
                        <titleStmt>
                            <title>EAC-CPF (Encoding archival context - Corporate bodies, persons,
                                families)</title>
                            <respStmt>
